...
export const enhancementStatus = pgEnum("enhancement_status", ['PENDING', 'PROCESSING', 'COMPLETED', 'FAILED'])
...
export const verification = pgTable("verification", {
	...
	createdAt: timestamp("created_at", { precision: 3, mode: 'date' }).default(sql`CURRENT_TIMESTAMP`),
	updatedAt: timestamp("updated_at", { precision: 3, mode: 'date' }).default(sql`CURRENT_TIMESTAMP`),
});
...
export const payment = pgTable("payment", {
	...
	index("payment_created_at_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamp_ops")),
	index("payment_status_idx").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("payment_stripe_checkout_session_id_idx").using("btree", table.stripeCheckoutSessionId.asc().nullsLast().op("text_ops")),
	uniqueIndex("payment_stripe_checkout_session_id_key").using("btree", table.stripeCheckoutSessionId.asc().nullsLast().op("text_ops")),
...
export const bodyProfile = pgTable("body_profile", {
	...
	uniqueIndex("body_profile_enhanced_photo_id_key").using("btree", table.enhancedPhotoId.asc().nullsLast().op("text_ops")),
	uniqueIndex("body_profile_photo_id_key").using("btree", table.photoId.asc().nullsLast().op("text_ops")),
	index("body_profile_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const creditTransaction = pgTable("credit_transaction", {
	...
	index("credit_transaction_created_at_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamp_ops")),
	uniqueIndex("credit_transaction_try_on_id_type_key").using("btree", table.tryOnId.asc().nullsLast().op("text_ops"), table.type.asc().nullsLast().op("enum_ops")),
	index("credit_transaction_type_idx").using("btree", table.type.asc().nullsLast().op("enum_ops")),
	index("credit_transaction_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	index("credit_transaction_wallet_id_idx").using("btree", table.walletId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const creditWallet = pgTable("credit_wallet", {
	...
	index("credit_wallet_balance_idx").using("btree", table.balance.asc().nullsLast().op("int4_ops")),
	index("credit_wallet_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	uniqueIndex("credit_wallet_user_id_key").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const styleTip = pgTable("style_tip", {
	id: text().primaryKey().notNull(),
...
export const tryOn = pgTable("try_on", {
	id: text().primaryKey().notNull(),
	userId: text("user_id").notNull(),
	...
	uniqueIndex("try_on_back_photo_id_key").using("btree", table.backPhotoId.asc().nullsLast().op("text_ops")),
	uniqueIndex("try_on_front_photo_id_key").using("btree", table.frontPhotoId.asc().nullsLast().op("text_ops")),
	uniqueIndex("try_on_result_id_key").using("btree", table.resultId.asc().nullsLast().op("text_ops")),
	index("try_on_status_idx").using("btree", table.status.asc().nullsLast().op("text_ops")),
	index("try_on_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const lookbookItem = pgTable("lookbook_item", {
	id: text().primaryKey().notNull(),
	...
	uniqueIndex("lookbook_item_lookbook_id_try_on_id_key").using("btree", table.lookbookId.asc().nullsLast().op("text_ops"), table.tryOnId.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const lookbook = pgTable("lookbook", {
	...
	uniqueIndex("lookbook_cover_id_key").using("btree", table.coverId.asc().nullsLast().op("text_ops")),
	index("lookbook_share_slug_idx").using("btree", table.shareSlug.asc().nullsLast().op("text_ops")),
	uniqueIndex("lookbook_share_slug_key").using("btree", table.shareSlug.asc().nullsLast().op("text_ops")),
	index("lookbook_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const garment = pgTable("garment", {
	...
	index("garment_brand_idx").using("btree", table.brand.asc().nullsLast().op("text_ops")),
	index("garment_category_idx").using("btree", table.category.asc().nullsLast().op("text_ops")),
	uniqueIndex("garment_image_id_key").using("btree", table.imageId.asc().nullsLast().op("text_ops")),
	index("garment_is_public_idx").using("btree", table.isPublic.asc().nullsLast().op("bool_ops")),
	uniqueIndex("garment_mask_id_key").using("btree", table.maskId.asc().nullsLast().op("text_ops")),
	index("garment_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const file = pgTable("file", {
	...
	uniqueIndex("file_key_key").using("btree", table.key.asc().nullsLast().op("text_ops")),
]);
