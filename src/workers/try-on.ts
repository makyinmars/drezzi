...
const modelName = "google/gemini-3-pro-image";
...
async function processRecord(record: SQSRecord): Promise<void> {
  ...
  const prompt = `Virtual try-on: Place the garment from the second image onto the person in the first image.
...
The result should look like a professional fashion photo.`;
  ...
  const result = await generateText({
    ...
    interaction?.finish({
      ...
      userId,
      convoId: tryOnId,
      output: `Try-on generation failed: ${message}`,
      properties: {
        status: "failed",
      },
    });
  ...
  if (imageFiles.length === 0) {
    interaction?.finish({
      ...
      userId,
      convoId: tryOnId,
      output: "Try-on generation failed: no image was returned by the model",
      properties: {
        status: "failed",
      },
    });
  ...
  interaction?.finish({
    ...
    userId,
    convoId: tryOnId,
    output: `Try-on generation completed (${generatedImage.mediaType ?? "image/png"})`,
    properties: {
      status: "completed",
    },
  });
